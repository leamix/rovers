...
class CommandParserTest extends \PHPUnit_Framework_TestCase
{
	...
	/**
	 * @expectedException \app\CommandException
	 * @expectedExceptionMessage Plato dimensions are invalid.
	...
	/**
	 * @expectedException \app\CommandException
	 * @expectedExceptionMessageRegExp #Rover \#\d+ (instructions are|position is) invalid.#
	...
	{
		$parser = new CommandParser($data);
		$this->assertEquals($parser->getPlatoDimensions(), $dimensions);
		$this->assertEquals($parser->getRoversPositions(), $roversPos);
		$this->assertEquals($parser->getRoversInstructions(), $roversInstr);
	}
